...
public class FileHandler implements AutoCloseable {
    ...
    // TODO: add clean past encode/decode file later..
    ...
    public void encodeProcessAfter(FileEncodeData fileEncodeData, long partId, int[] y) {
        ...
        for (int i = 0; i < TOTAL_PACKET_PER_PART; i++) {
            ...
            Thread.ofVirtual().start(() -> {
                ...
                // TODO: may need move this to callback function..
    ...
    public void decodeProcessAfter(FileDecodeData fileDecodeData, long partId, int[] p) {
        ...
        fileHandlerCallback.decodePartProcessAfter(fileDecodeData);
    ...
    public void encode(FileEncodeData fileEncodeData, byte[] data) {
        for (int i = 0; i < fileEncodeData.getNumOfPart(); i++) {
            ...
            Thread.ofVirtual().start(() -> {
                ...
                rsCode.encode(fileEncodeData, partId, symbols);
            });
        }
    }

    public void decode(FileDecodeData fileDecodeData, PacketHandler packetHandler) {
        Thread.ofVirtual().start(() -> {
            try {
                if (!packetHandler.verify()) return;
            } catch (NoSuchAlgorithmException e) {
                throw new RuntimeException(e);
            }
            ...
            if (partDecodeData.getNumOfReceivedPacket().incrementAndGet() == PACKET_NEED_PER_PART) {
                ...
                for (int i = 0; i < TOTAL_PACKET_PER_PART; i++) {
                    if (receivedPacket[i] == null) continue;
                    int st = i * SYMBOL_PER_PACKET;
                    int st_x = i * SEG_PER_PACKET;
                    System.arraycopy(receivedPacket[i].getSymbols(), 0, y, st, SYMBOL_PER_PACKET);
                    for (int j = 0; j < SEG_PER_PACKET; j++) {
                        x[st + j] = st_x + j;
                        x[st + j + SEG_PER_PACKET] = st_x + j + SEG_DIFF;
                    }
                }
                rsCode.decode(fileDecodeData, packetHandler.getPartId(), x, y);
            }
        });
    }
    ...
    public static void genBytesFromDecodedSymbols(int[] symbols, byte[] data, int offset) {
        ByteBuffer buffer = ByteBuffer.wrap(data, offset, PacketHandler.DATA_BYTE_SIZE);
    ...
    public class FileDecodeData {
        ...
        public FileDecodeData(UUID fileId, long fileByteCnt) {
            ...
            fileBytes = new byte[(int) fileByteCnt];
