...
__host__ __forceinline__ __device__ inline void fnt(unsigned *a, unsigned *b, unsigned log_na, unsigned log_nb, bool inv,
	unsigned *d_N_pos, unsigned *d_root_layer_pow, unsigned *d_inv)
{
	...
	unsigned na = 1 << log_na, nb = 1 << log_nb;
	unsigned n_th, n_bl;
	...
	for (unsigned i = 0; i < log_nb; i++)
		fnt_i CUDA_KERNEL(n_bl, n_th, NULL, NULL)(a, b, i, inv, d_root_layer_pow);
...
__global__ void end_poly_mul_large(unsigned* t1, unsigned* t2, unsigned* c, unsigned log_n,
	...
	fnt(c, c, log_n + 1, log_n + 1, 1, d_N_pos, d_root_layer_pow, d_inv);
...
void test_fnt() {
	...
	for (unsigned tt = 0; tt < N_test; tt++) {
		...
		fnt(d_v, d_c2, log_nv, log_nv, 1, d_N_pos, d_root_layer_pow, d_inv);
