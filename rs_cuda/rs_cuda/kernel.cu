...
const unsigned ALGO_N_1_LOW_WPT = 1024; // poly_mul
	...
	unsigned* d_N_pos, unsigned* d_root_layer_pow, unsigned* d_inv, unsigned wpt)
{
	...
	*/
	...
	unsigned na = 1 << log_na, nb = 1 << log_nb, wp = (opt & 2) >> 1, st = nb - 1, offset = nb >> 1;
	for (unsigned j = 0; j < wpt; j++) {
		unsigned k = id * wpt + j;
		if (k < na) b[d_N_pos[st + k]] = a[k];
		if (log_na == log_nb) b[d_N_pos[st + k + offset]] = a[k + offset];
	}
	...
	for (unsigned i = 0; i < log_nb; i++) {
		...
		for (unsigned j = 0; j < wpt; j++) {
			...
			unsigned u = b[pos];
			unsigned v = mul_mod(b[pos + haft_len], d_root_layer_pow[(LEN_ROOT_LAYER_POW * wp) + haft_len - 1 + th_id]);
			b[pos] = add_mod(u, v);
	...
	if (opt & 1) {
		for (unsigned j = 0; j < wpt; j++) {
			unsigned k = id * wpt + j;
			b[k << 1] = div_mod(b[k << 1], nb, d_inv);
			b[(k << 1) | 1] = div_mod(b[(k << 1) | 1], nb, d_inv);
		}
	...
	unsigned* d_N_pos, unsigned* d_root_layer_pow, unsigned* d_inv)
{
	...
	for (unsigned i = 0; i < log_nb; i++) {
		...
		unsigned wlen = d_root_layer_pow[(LEN_ROOT_LAYER_POW * wp) + haft_len - 1];
	...
	unsigned* d_N_pos, unsigned* d_root_layer_pow, unsigned* d_inv)
{
	...
	else
	{
		...
		if (na <= ALGO_N_1_KERNEL_CUTOFF) {
			...
			// may try create stream here..
			unsigned n_th1, wpt1 = ALGO_N_1_HIGH_WPT;
			build_launch_param(na, wpt1, n_th1);
			g_fnt CUDA_KERNEL(1, n_th1, NULL, NULL) (a, t1, log_n, log_n + 1, 0, d_N_pos, d_root_layer_pow, d_inv, wpt1);
			g_fnt CUDA_KERNEL(1, n_th1, NULL, NULL) (b, t2, log_n, log_n + 1, 0, d_N_pos, d_root_layer_pow, d_inv, wpt1);

			unsigned n_th2, wpt2 = ALGO_N_1_LOW_WPT;
			build_launch_param(nc, wpt2, n_th2);
			g_vector_mul_i CUDA_KERNEL(1, n_th2, NULL, NULL) (t1, t2, t1, wpt2);

			g_fnt CUDA_KERNEL(1, n_th1, NULL, NULL) (t1, c, log_n + 1, log_n + 1, 3, d_N_pos, d_root_layer_pow, d_inv, wpt1);
